...
<div class="d-flex justify-content-center my-4" style="margin-left: 35px;">
  <div class="btn-group" role="tablist">
    ...
    <a href="{{ route('docRecetaCrear', $mascota->id) }}" class="btn btn-outline-primary " role="tab">
      Crear Receta
    </a>
...
<div class="receta-container" >
    ...
    <div class="info-paciente">
        Paciente: <span>{{ $mascota->nombre }}</span><br>
        Fecha de la última receta: 
        <span>{{ \Carbon\Carbon::parse($recetas->last()->Fecha)->format('d/m/Y') }}</span>
    </div>
        ...
        <tbody>
            @foreach ($recetas as $receta)
                <tr>
                    ...
                    <td>{{ $receta->nom_producto }}</td>
                    <td>{{ $receta->Dosis }}</td>
                    <td>{{ $receta->Cuidados }}</td>
                    <td>{{ $receta->Fecha }}</td>
                </tr>
            @endforeach
        </tbody>
...
@else
    <p>No hay receta registrada para esta mascota.</p>
@endif
    <div class="contacto">
      Tel: <span>9999 9999</span><br />
      Email: <span>VitalVet@gmail.com</span>
    </div>
</div>
    <center>
        <button onclick="window.print()" class="btn btn-secondary mb-3" style="border-radius: 0%" >
            Imprimir Receta
        </button>
    </center>
